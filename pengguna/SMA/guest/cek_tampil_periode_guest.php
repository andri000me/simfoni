...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_semester = $guest->cek_periodeguest($_POST['id_semester']);
				if ($id_semester == "kosong") 
				{
					echo "<script>alert('Data semester tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_periode_guest' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_periode_guest&id_semester=$id_semester' </script>";
				}
