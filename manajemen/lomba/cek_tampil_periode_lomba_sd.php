...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_tp = $lomba->cek_periodelomba_sd($_POST['id_tp']);
				if ($id_tp == "kosong") 
				{
					echo "<script>alert('Data Tahun Pelajaran tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_periode_lomba_sd' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_periode_lomba_sd&id_tp=$id_tp' </script>";
				}
