...
<div class="row">
	<div class="col-md-6">
<h3>Pilih Data Tahun Pelajaran Lomba</h3>
<hr>
...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_tp = $lomba->cek_periodelombasmp($_POST['id_tp']);
				if ($id_tp == "kosong") 
				{
					echo "<script>alert('Data Tahun Pelajaran tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_periode_lomba_smp' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_periode_lomba_smp&id_tp=$id_tp' </script>";
				}
