...
<?php 
	...
	$data_kelas_sd = $kelas->tampil_kelas_smp();
...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_kelas = $guest->cek_periodekelas_smp($_POST['id_kelas']);
				if ($id_kelas == "kosong") 
				{
					echo "<script>alert('Data kelas tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_siswa_smp' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_siswa_smp&id_kelas=$id_kelas' </script>";
				}
