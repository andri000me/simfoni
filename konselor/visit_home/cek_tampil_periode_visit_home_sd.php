...
<div class="row">
	<div class="col-md-6">
<h3>Pilih Data Semester Home Visit SD</h3>
<hr>
...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_semester = $visit_home->cek_periodevisit_home_sd($_POST['id_semester']);
				if ($id_semester == "kosong") 
				{
					echo "<script>alert('Data semester tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_periode_visit_home_sd' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_periode_visit_home_sd&id_semester=$id_semester' </script>";
				}
