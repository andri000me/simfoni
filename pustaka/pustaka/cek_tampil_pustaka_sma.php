...
<pre>
	<?php //print_r($data_tahun); ?>
		...
		<?php 
			...
			{
				$id_semester = $datapustaka->cek_periodedatapustaka_sma($_POST['id_semester']);
				if ($id_semester == "kosong") 
				{
					echo "<script>alert('Data semester tidak ada data') </script>";
					echo "<script>location='index.php?halaman=cek_tampil_pustaka_sma' </script>";

				} else {
					echo "<script>location='index.php?halaman=tampil_periode_pustaka_sma&id_semester=$id_semester' </script>";
				}
